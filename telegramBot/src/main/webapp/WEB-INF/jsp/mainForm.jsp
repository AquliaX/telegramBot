...
<head>
    ...
    <script>
        ...
        function chatbotResponse() {
            talking = true;
            botMessage = "Sorry, I don't understand you. Check !commands"; //the default message

            for (let i = 0; i < townsName.length; ++i) {
                if (lastUserMessage === townsName[i]) {
                    botMessage = townsInfo[i];
                    break;
                }
            }
            if (lastUserMessage === '!commands') {
                botMessage = 'commands : !towns, !hi, !name.'
            }
            if (lastUserMessage === '!towns') {
                botMessage = 'List of available cities: ';
                for (let i = 0; i < townsName.length; ++i) {
                    botMessage +=', '+ townsName[i] ;
                }
                botMessage += '. To learn more about the city, write me its name.'
            }
